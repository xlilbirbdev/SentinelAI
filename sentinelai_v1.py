...
Real-time terminal dashboard with four panels:
  - PROCESSES   newly spawned + top resource users
  - NETWORK     active inet connections + owning process
  - AUTH        recent login / sudo / privilege events (platform-specific)
  - FILESYSTEM  create / modify / delete / move events in a watched dir

psutil and watchdog are cross-platform, so PROCESSES, NETWORK and FILESYSTEM
need no per-OS code. AUTH is the one panel that differs per OS, so it's
isolated behind the AuthReader strategy interface below:

    class AuthReader:
        def available(self) -> bool          # can we read events right now?
        def read(self, n=12) -> list[(label, text)]   # oldest -> newest
...
# ===========================================================================
# Filesystem watcher - runs on a background thread (watchdog picks inotify /
# FSEvents / ReadDirectoryChangesW automatically). It only ever appends to a
# deque; deque.append is atomic in CPython, so the UI thread can safely drain
# it on each refresh tick without a lock.
# ===========================================================================
...
# ===========================================================================
# Auth readers - one per OS, behind a shared interface
# ===========================================================================
class AuthReader:
...
class LinuxAuthReader(AuthReader):
    ...
    def _classify(line: str) -> tuple[str, str]:
        # Trim the leading "timestamp host" prefix for readability.
...
class SentinelAI(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Grid(id="grid"):
            ...
            yield self._panel("net", "NETWORK CONNECTIONS",
    ...
    @staticmethod
    def _panel(table_id: str, title: str, columns: tuple[str, ...]) -> Vertical:
        table = DataTable(id=table_id, zebra_stripes=True, cursor_type="none")
        table.add_columns(*columns)
        return Vertical(Static(title, classes="title"), table, classes="panel")
    ...
    def on_mount(self) -> None:
        ...
        # Prime CPU counters so the first reading isn't all zeros.
    ...
    def _refresh_processes(self) -> None:
        ...
        # Newly spawned first, then highest CPU.
    ...
    def _refresh_network(self) -> None:
        table = self.query_one("#net", DataTable)
        table.clear()
        try:
            conns = psutil.net_connections(kind="inet")
        except (psutil.AccessDenied, PermissionError):
            import platform
            hint = "run as Administrator" if platform.system() == "Windows" else "run with sudo"
            table.add_row("-", "permission denied", hint, "-", "-", "-")
            return
        ...
        for c in conns:
            ...
            if c.pid:
                try:
                    ...
                    pass
            table.add_row(proto, laddr, raddr, c.status, str(c.pid or "-"), pname)
            shown += 1
            if shown >= 14:
                break
...
# ===========================================================================
def main():
    ap = argparse.ArgumentParser(description="SentinelAI host monitor")
